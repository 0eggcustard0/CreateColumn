...
namespace CreateColumn
{
    ...
    public class AutoCreate : IExternalCommand
    {
        ...
        public static string FirstDictrule = "";
        public static string SecondDictrule = "";
        ...
        public static void Reset()
        {
            ...
            FirstDictrule = @"([A-Za-z]+\d+\s*(?:~\s*[A-Za-z]+\d+)?)";
            SecondDictrule = @"([A-Za-z]+\d+\s*)(?:~|-)([A-Za-z]+\d+\s*)";
            ...
            try
            {
                using (var fs = File.OpenRead(path))
                {
                    ...
                    foreach (Entity entity in document.Entities)
                    {
                        ...
                        {
                            CSMath.XYZ adjustedPosition = GetTextCenterPosition(text);
        ...
        public static CSMath.XYZ GetTextCenterPosition(TextEntity text)
        {
            // 假設大多數文字是預設對齊 (Left + Baseline)
            CSMath.XYZ basePoint = text.InsertPoint;
            double height = text.Height;
            double rotation = text.Rotation;

            // 從基線移到垂直中心
            double verticalOffset = height * 0.5;  // 基線大約在 40% 高度
            ...
            return new CSMath.XYZ(
                basePoint.X - verticalOffset * sin,
                basePoint.Y + verticalOffset * cos,
                basePoint.Z
            );
        }
        ...
        public static Result getTextdiction()
        {
            ...
            foreach (TextInfo size in DictText.Where(i => Regex.Match(i.Content, pattern, RegexOptions.IgnoreCase).Success))
            {
                foreach (TextInfo text in DictText.Where(j => Regex.Match(j.Content, FirstDictrule, RegexOptions.IgnoreCase).Success || Regex.Match(j.Content, BeaminFloor).Success))
                {
                    if (text == size) continue;
                    if (Math.Abs(size.Position.Y - text.Position.Y) <= 50 && Math.Abs(size.Position.X - text.Position.X) <= 2000)
                    {
                        var match1 = Regex.Matches(text.Content, FirstDictrule);

                        foreach (Match match in match1)
                        {
                            try
                            {
                                var match2 = Regex.Match(match.Groups[1].Value, SecondDictrule);
                                if (match2.Success == true)                                             //有~或-做連號的
                                {
                                    var code = Regex.Match(match2.Groups[1].Value, @"([A-Za-z]+)(\d*)").Groups[1].Value;
                                    var start = Regex.Match(match2.Groups[1].Value, @"([A-Za-z]+)(\d*)").Groups[2].Value;
                                    var end = Regex.Match(match2.Groups[2].Value, @"([A-Za-z]+)(\d*)").Groups[2].Value;
                                    int startValue, endValue;
                                    if (int.TryParse(start, out startValue))
                                    {
                                        var startNumber = startValue;
                                    }
                                    if (int.TryParse(end, out endValue))
                                    {
